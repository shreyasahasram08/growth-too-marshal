...
def create_all():
    ...
    # FIXME: are there more program ids for DECam and Gattini?
    program_ids = {"ZTF": [1, 2, 3], # program id 0 == all program ids
                   "Gattini": [1],
                   "DECam": [1],
                   "KPED": [1],
                   "GROWTH-India": [1]}
...
class ObservationList(db.Model):
    ...
    def ipix(self, filt, program_id, start_time, end_time):
        observation_list = self.get_observations(filt, program_id,
                                             start_time, end_time)
        return {
    ...
    def area(self, filt, program_id, start_time, end_time):
        nside = Localization.nside
        ipix = self.ipix(filt, program_id, start_time, end_time)
        return hp.nside2pixarea(nside, degrees=True) * len(ipix)

    def nexp(self, filt, program_id, start_time, end_time):
        observation_list = self.get_observations(filt, program_id,
                                             start_time, end_time)
        observation_ids = []
    ...
    def starttime(self, filt, program_id, start_time, end_time):
        observation_list = self.get_observations(filt, program_id,
                                             start_time, end_time)
    ...
    def totaltime(self, filt, program_id, start_time, end_time):
        observation_list = self.get_observations(filt, program_id,
                                             start_time, end_time)
        tot = 0
        observation_ids = []
    ...
    def limmag(self, filt, program_id, start_time, end_time):
        observation_list = self.get_observations(filt, program_id,
                                             start_time, end_time)
        limmags = []
        for ii, observation in enumerate(observation_list):
    ...
    def get_probability(self, filt, program_id, start_time, end_time):
        localization = Localization.query.filter_by(
            dateobs=self.dateobs,
            localization_name=Localization.localization_name).one()

        ipix = np.asarray(list(self.ipix(filt, program_id, start_time, end_time)))
        if len(ipix) > 0:
            return localization.flat_2d[ipix].sum()
        else:
            return 0.0

    def get_observations(self, filt, program_id, start_time, end_time):
        ...
        if filt == "a" and program_id ==0:
            ...
            observations = Observation.query.filter(
                ...
                (Observation.filter_id == filter_id) &
                (Observation.program_id == program_id)).all()
